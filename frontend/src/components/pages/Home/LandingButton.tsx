...
export default function LandingButton(props: LandingButtonProps) {
  return (
    <div>
      ...
      {props.type === 'go' && (
        <button
          className="bg-[var(--orange-color)] text-[var(--primary-color)] font-semibold py-2 px-6 rounded-full text-sm hover:bg-[var(--blue-color)] hover:text-[var(--dark-text)] transition-colors cursor-pointer"
        >
          {props.text}
        </button>
      )}
